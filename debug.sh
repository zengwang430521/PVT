...
srun -p pat_earth \
    ...
    --job-name=pvt --ntasks=16 \
    --gres=gpu:8 --ntasks-per-node=8 --cpus-per-task=5 --kill-on-bad-exit=1 \
      python -u train.py --model mypvt24_small --batch-size 64 --epochs 300 --num_workers 5  --cache_mode \
    --output_dir work_dirs/my24 --data-path data/imagenet \
    --input-size 448 \
    --resume work_dirs/my24/checkpoint.pth
