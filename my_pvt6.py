...
def token2map(x, loc, map_size, kernel_size, sigma, conf=None):
    H, W = map_size
    B, N, C = x.shape
    loc = loc.clamp(0, 1)
...
class MyAttention(nn.Module):
    ...
    def forward(self, x, x_source, loc_source, H, W, conf_source=None):
        ...
        if self.sr_ratio > 1:
            x_source = token2map(x_source, loc_source, [H, W], self.sr_ratio-1, 2, conf_source)
...
class MyBlock(nn.Module):
    ...
    def forward(self, x, x_source, loc, H, W):
        if self.use_fc:
            x = self.fc(x) + self.drop_path(self.attn(self.norm1(x), self.norm1(x_source), loc, H, W)) * self.alpha
        else:
            x = x + self.drop_path(self.attn(self.norm1(x), self.norm1(x_source), loc, H, W)) * self.alpha
...
class DownLayer(MyBlock):
    """ Down sample
    """
    def __init__(self, sample_num, embed_dim, drop_rate, down_block_cfg):
        super().__init__(**down_block_cfg)
    ...
    def forward(self, x, pos, pos_embed, H, W, pos_size, N_grid):
        ...
        x_grid = x[:, :N_grid]
        x_ada = x[:, N_grid:]
        x_grid_ori = x_ori[:, :N_grid]
        x_ada_ori = x_ori[:, N_grid:]
        pos_grid = pos[:, :N_grid]
        pos_ada = pos[:, N_grid:]
        ...
        x_down_ori = torch.gather(x_ada_ori, 1, index_down.expand([B, self.sample_num, C]))
        pos_down = torch.gather(pos_ada, 1, index_down.expand([B, self.sample_num, 2]))

        x_down = torch.cat([x_grid, x_down], 1)
        x_down_ori = torch.cat([x_grid_ori, x_down_ori], 1)
        pos_down = torch.cat([pos_grid, pos_down], 1)

        if self.use_fc:
            x_down = self.fc(x_down_ori) + self.drop_path(self.attn(x_down, x, pos, H, W, conf)) * self.alpha
        else:
            x_down = x_down_ori + self.drop_path(self.attn(x_down, x, pos, H, W, conf)) * self.alpha

        x_down = x_down + self.drop_path(self.mlp(self.norm2(x_down))) * self.alpha
...
class MyPVT6(nn.Module):
    ...
    def reset_drop_path(self, drop_path_rate):
        ...
        cur += self.depths[0]
        self.down_layers1.drop_path.drop_prob = dpr[cur]
        cur += 1
        ...
        cur += self.depths[1] - 1
        self.down_layers2.drop_path.drop_prob = dpr[cur]
        cur += 1
        ...
        cur += self.depths[2] - 1
        self.down_layers3.drop_path.drop_prob = dpr[cur]
        cur += 1
...
@register_model
def mypvt6_small(pretrained=False, **kwargs):
    model = MyPVT6(
...
if __name__ == '__main__':
    ...
    model = mypvt6_small(drop_path_rate=0.1).to(device)
