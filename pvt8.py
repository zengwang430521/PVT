...
def reconstruct_feature(feature, mask, kernel_size, sigma):
    ...
    mask_inter = (mask_inter > 0).float().type(feature.dtype)
...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.sr_ratio > 1:
            x_ = token2map(x, loc, [int(H//self.sr_ratio), int(W//self.sr_ratio)], 3, 2)
        else:
            x_ = x
...
@register_model
def pvt8_small(pretrained=False, **kwargs):
...
if __name__ == '__main__':
    ...
    model = pvt8_small(drop_path_rate=0.1).to(device)
