...
class DownLayer(nn.Module):
    ...
    def forward(self, x, pos_orig, idx_agg, agg_weight, H, W, N_grid):
        ...
        idx_agg_down = index_points(idx_agg_down[..., None], idx_agg).squeeze(-1)


        x = map2token_agg_fast_nearest(x_map, H*W, pos_orig, idx_agg_down, agg_weight) # + self.conv_skip(x)
        # xt = x_map.flatten(2).permute(0,2,1)
        x = self.norm(x)
        conf = self.conf(x)
        weight = conf.exp()
        ...
        # mean_weight = F.interpolate(mean_weight, [H*2, W*2], mode='nearest')
        ...
        agg_weight_down = agg_weight

        idx_agg, agg_weight = idx_agg_down, agg_weight_down
...
@register_model
def mypvt3h2_density0d_small(pretrained=False, **kwargs):
    model = MyPVT(
...
@register_model
def mypvt3h2_density0d_large(pretrained=False, **kwargs):
    model = MyPVT(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
@register_model
def mypvt3h2_density0d_tiny(pretrained=False, **kwargs):
    model = MyPVT(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
