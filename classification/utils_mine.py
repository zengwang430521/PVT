...
def show_tokens_merge_00(x, out, N_grid=14*14):
    # import matplotlib.pyplot as plt
    IMAGENET_DEFAULT_MEAN = torch.tensor([0.485, 0.456, 0.406], device=x.device)[None, :, None, None]
    IMAGENET_DEFAULT_STD = torch.tensor([0.229, 0.224, 0.225], device=x.device)[None, :, None, None]
    x = x * IMAGENET_DEFAULT_STD + IMAGENET_DEFAULT_MEAN
...
def token_cluster_density_fixbug_light(x, Ns, idx_agg, weight=None, return_weight=False, conf=None,
                          k=5, dist_assign=False, ada_dc=False, use_conf=False, conf_scale=0.25,
                          conf_density=False, xf=None):
    # import torch
    # x = torch.rand(2, 1000, 64)
    # Ns = 250
    # k = 3
...
def get_merge_way(input_dict, Ns):
    x = input_dict['x']
...
def token_cluster_density_fixbug_sr(x, Ns, idx_agg, weight=None, return_weight=False, conf=None,
                          k=5, dist_assign=False, ada_dc=False, use_conf=False, conf_scale=0.25,
                          conf_density=False, map_size=[14, 14], loc_orig=None, agg_weight=None):
    # import torch
    # x = torch.rand(2, 1000, 64)
    # Ns = 250
    # k = 3
    ...
    #
    # plt.subplot(2, 4, 1)
    # plt.imshow(dist_matrix[0].detach().cpu())
    # plt.subplot(2, 4, 2)
    # plt.imshow(token2map_agg_sparse(density[:,:,None], None, loc_orig, idx_agg, [56, 56])[0].detach().cpu()[0, 0])
    # plt.subplot(2, 4, 3)
    # plt.imshow(token2map_agg_sparse(dist[:,:,None], None, loc_orig, idx_agg, [56, 56])[0].detach().cpu()[0, 0])
    # plt.subplot(2, 4, 4)
    # plt.imshow(token2map_agg_sparse(score[:,:,None], None, loc_orig, idx_agg, [56, 56])[0].detach().cpu()[0, 0])
    #
    #
    # with torch.no_grad():
    #     h, w = map_size
    ...
    #     dist_matrix = map2token_agg_sparse_nearest(dist_map, N, loc_orig, idx_agg, agg_weight)
    #     dist_matrix = (dist_matrix + dist_matrix.permute(0, 2, 1)) / 2
    #     idx_tmp = torch.arange(N, device=x.device)
    #     dist_matrix[:, idx_tmp, idx_tmp] = 0
    #
    #     # normalize dist_matrix for stable
