...
from utils_mine import token_cluster_density_fixbug_light as token_cluster_density
...
class DownLayer(nn.Module):
    ...
    def forward(self, x, pos_orig, idx_agg, agg_weight, H, W, N_grid):
        ...
        x_down, idx_agg_down, weight_t = token_cluster_density(
            x[:, :, :C_old], sample_num, idx_agg, weight, True, conf,
            k=self.k, dist_assign=self.dist_assign, ada_dc=self.ada_dc,
            use_conf=self.use_conf, conf_scale=self.conf_scale, conf_density=self.conf_density, xf=x)
...
@register_model
def mypvt3h2_density0fl3_small(pretrained=False, **kwargs):
    model = MyPVT(
...
@register_model
def mypvt3h2_density0fl3_large(pretrained=False, **kwargs):
    model = MyPVT(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
@register_model
def mypvt3h2_density0fl3_tiny(pretrained=False, **kwargs):
    model = MyPVT(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
