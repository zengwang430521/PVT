...
'''
do not select tokens, merge tokens. weight = None
'''
...
class DownLayer(nn.Module):
    ...
    def forward(self, x, pos, H, W, N_grid):
        ...
        # weight = conf.exp()
        x_down, pos_down = merge_tokens(x, pos, pos_down, None)
...
@register_model
def mypvt3f2_small(pretrained=False, **kwargs):
    model = MyPVT(
...
if __name__ == '__main__':
    ...
    model = mypvt3f2_small(drop_path_rate=0.).to(device)
