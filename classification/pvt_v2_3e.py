        ...
        for i in range(1, num_stages):
            down_layers = DownLayer(sample_ratio=0.25, embed_dim=embed_dims[i-1], dim_out=embed_dims[i],
                                          ...
                                          down_block=MyBlock(
                                              ...
                                              norm_layer=norm_layer, sr_ratio=1, linear=linear)
                                    )
...
@register_model
def mypvt3e_small(pretrained=False, **kwargs):
    model = MyPVT(
...
if __name__ == '__main__':
    ...
    model = mypvt3e_small(drop_path_rate=0.).to(device)
