...
class Saliency_Sampler(nn.Module):
    def __init__(self, task_network, saliency_network, task_input_size, saliency_input_size):
        ...
        self.conv_last = nn.Conv2d(1, 1, kernel_size=1, padding=0, stride=1)
...
model = Saliency_Sampler(None, nn.Conv2d(3, 1, kernel_size=1), 56, 56)
model.to(device)

y = model(torch.zeros(1, 3, 56, 56).to(device), 0)