...
from pvt_v2_norm16 import (BlockNorm, DropPath, DWConv, OverlapPatchEmbed,
...
class MyMlp(nn.Module):
    ...
    def forward(self, x, loc_orig, idx_agg, agg_weight, H, W):
        x = self.fc1(x)
        x = self.act(self.norm1(x)).type(x.dtype)
        if self.linear:
            x = self.relu(x)

        x = self.dwconv(x, loc_orig, idx_agg, agg_weight, H, W)

        x = self.act(self.norm2(x)).type(x.dtype)
        ...
        x = self.fc2(x)
        # x = self.drop(x)
        x = self.act(self.norm3(x)).type(x.dtype)
        return x
    ...
    def forward(self, x, loc_orig, x_source, idx_agg_source, H, W, conf_source=None):
        ...
        if not self.linear:
            if self.sr_ratio > 1:
                ...
                x_source = self.norm(x_source).type(x_source.dtype)
...
class MyBlock(nn.Module):
    ...
    def forward(self, x, idx_agg, agg_weight, loc_orig,
        ...
        x1 = x + self.drop_path(self.attn(self.norm1(x).type(x.dtype),
                                          loc_orig,
                                          self.norm1(x_source).type(x_source.dtype),
                                          idx_agg_source,
                                          H, W, conf_source))

        x2 = x1 + self.drop_path(self.mlp(self.norm2(x1).type(x1.dtype),
                                          loc_orig,
                                          idx_agg,
                                          agg_weight,
                                          H, W))
...
@register_model
def mypvt3h2_density0_norm16_small(pretrained=False, **kwargs):
    model = MyPVT(
...
@register_model
def mypvt3h2_density25_norm16_small(pretrained=False, **kwargs):
    model = MyPVT(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1],
...
@register_model
def mypvt3h2_density50_norm16_small(pretrained=False, **kwargs):
    model = MyPVT(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1],
...
if __name__ == '__main__':
    ...
    model = mypvt3h2_density0_norm16_small(drop_path_rate=0.).to(device)
    import time
