...
from utils_mine import (
    ...
    show_tokens, show_conf, merge_tokens,  conf_resample
)
...
class MyPVT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        _, loc_grid, N_grid = get_loc(x, H, W, self.grid_stride)

        x_hr, H_hr, W_hr = self.re_patch_embed(img)

        conf = self.conf(x)
        conf_map = conf.reshape(B, H, W, 1).permute(0, 3, 1, 2)
        conf_map = F.interpolate(conf_map, [H_hr, W_hr], mode='bilinear')
        loc_grid = loc_grid[:, :N_grid]
        loc_re = conf_resample(conf_map, N-N_grid)
        loc_re = torch.cat([loc_grid, loc_re], dim=1)

        x_re = map2token(x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous(), loc_re)
        conf_re = map2token(conf_map, loc_re)

        x_t = torch.cat([x, x_re], dim=1)
        loc_t = torch.cat([loc, loc_re], dim=1)
        conf_t = torch.cat([conf, conf_re], dim=1)
        weight_t = conf_t.clamp(-7, 7).exp()

        x_re, loc_re = merge_tokens(torch.cat([x_t, conf_t], dim=-1),
                                    loc_t, loc_re, weight_t)   # merge tokens
        conf_re = x_re[:, :, C:]
        x_re = x_re[:, :, :C]
