...
from utils_mine import (
    get_grid_loc,
    gumble_top_k,
    show_tokens_merge, show_conf_merge, merge_tokens_agg_qkv, token2map_agg_sparse, map2token_agg_mat
)
...
class DownLayer(nn.Module):
    ...
    def __init__(self, sample_ratio, embed_dim, dim_out, drop_rate, down_block):
        ...
        self.q = nn.Linear(self.dim_out, self.dim_out)
        self.k = nn.Linear(self.dim_out, self.dim_out)
    ...
    def forward(self, x, pos, pos_orig, idx_agg, agg_weight, H, W, N_grid):
        ...
        x_down, idx_agg_down, weight_t = merge_tokens_agg_qkv(q, k, v, index_down, idx_agg, weight, True)
...
@register_model
def mypvt3f15_small(pretrained=False, **kwargs):
    model = MyPVT(
...
if __name__ == '__main__':
    ...
    model = mypvt3f15_small(drop_path_rate=0.).to(device)
