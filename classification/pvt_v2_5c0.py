...
from utils_mine import (
    ...
    show_tokens, show_conf, get_sample_grid, get_sample_grid2
)
...
# vis = True
'''
Saliency-Based Sampling, weight map with inter,
init grid from last stage
'''
...
class DownLayer(nn.Module):
    ...
    def forward(self, x, pos, H, W, N_grid):
        ...
        weight, mask = token2map(weight, pos, [H, W], kernel_size=kernel_size, sigma=2, return_mask=True)

        pos_init = pos.reshape(B, Ho, Wo, 2).permute(0, 3, 1, 2)
        pos_init = F.avg_pool2d(pos_init, kernel_size=3, stride=2, padding=1).reshape(B, 2, -1).permute(0, 2, 1)
        pos_down = get_sample_grid2(weight.detach(), pos_init)
...
@register_model
def mypvt5c0_small(pretrained=False, **kwargs):
    model = MyPVT(
...
if __name__ == '__main__':
    ...
    model = mypvt5c0_small(drop_path_rate=0.).to(device)
