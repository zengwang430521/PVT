...
'''
modify down block
partical conv and partical inter, method=1
'''
...
class MyMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., linear=False):
        ...
        self.dwconv = MyTokenConv(hidden_features, groups=hidden_features)
        self.act = act_layer()
...
class MyTokenConv(nn.Module):
    ...
    def forward(self, x, loc, H, W, conf=None, return_token=True):
        x, mask = token2map_partical(x, loc, [H, W], conf=conf, method=1)
...
class MyAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, linear=False):
        ...
        if not linear:
            if sr_ratio > 1:
                ...
                self.sr = MyTokenConv(dim, dim, kernel_size=sr_ratio, stride=sr_ratio)
    ...
    def forward(self, x, x_source, loc_source, H, W, conf_source=None):
        ...
        if not self.linear:
            if self.sr_ratio > 1:
                x_source, conf_source = self.sr(x_source, loc_source, H, W, conf_source, return_token=False)
                x_source = x_source.reshape(B, C, -1).permute(0, 2, 1)
                conf_source = conf_source.reshape(B, 1, -1).permute(0, 2, 1)
                conf_source = conf_source.log()
                x_source = self.norm(x_source)
            else:
                pass
        else:
            x_source, conf_source = token2map_partical(x_source, loc_source, [H, W], conf_source, method=1)
...
class MyBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if self.sample_ratio < 1:
            self.pre_conv = MyTokenConv(dim_in, dim, kernel_size=3, stride=2, padding=1)
...
@register_model
def mypvt3c2_small(pretrained=False, **kwargs):
    model = MyPVT(
...
if __name__ == '__main__':
    ...
    model = mypvt3c2_small(drop_path_rate=0.).to(device)
