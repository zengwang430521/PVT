...
class ResampleBlock(nn.Module):
    ...
    def forward(self, x, loc, src, H, W, N_grid,  conf_old=None):
        ...
        return x_down, loc_down, conf_map
...
class MyPVT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for n, blk in enumerate(self.block2):
            x, loc, conf_map = blk(x, loc, img, H, W, N_grid, conf_map)
            if n == 0:
                H, W = H // 2, W // 2
        x = self.norm2(x)
...
@register_model
def mypvt26_small(pretrained=False, **kwargs):
    model = MyPVT(
...
@register_model
def mypvt26a_small(pretrained=False, **kwargs):
    model = MyPVT26a(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], **kwargs)
    model.default_cfg = _cfg()
...
if __name__ == '__main__':
    ...
    model = mypvt26_small(drop_path_rate=0.1).to(device)
    ...
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    model = mypvt26a_small(drop_path_rate=0.1).to(device)
    model.reset_drop_path(0.1)
