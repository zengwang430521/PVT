...
class DownLayer(nn.Module):
    ...
    def __init__(self, sample_num, embed_dim, drop_rate, down_block):
        ...
        self.T_min = 0.01
        self.T_decay = 0.9998
        self.conv = nn.Conv2d(embed_dim, self.block.dim_out, kernel_size=2, stride=2, padding=0)
    ...
    def forward(self, x, pos, pos_embed, H, W, pos_size, N_grid):
        ...
        locd_grid = xy_map.reshape(-1, 2)[None, ...].repeat([B, 1, 1])
        ...
        index_down = gumble_top_k(conf, self.sample_num, 1, T=T)
        # conf = F.softmax(conf, dim=1) * N
        # conf = F.sigmoid(conf)
        # conf = self.gumble_sigmoid(conf)

        xd_ada = torch.gather(x, 1, index_down.expand([B, self.sample_num, C]))
        locd_ada = torch.gather(pos, 1, index_down.expand([B, self.sample_num, 2]))
        xd_ada = self.block(xd_ada, x, pos, H, W, conf)

        x_down = torch.cat([xd_grid, xd_ada], 1)
        pos_down = torch.cat([locd_grid, locd_ada], 1)
...
@register_model
def mypvt15_small(pretrained=False, **kwargs):
    model = MyPVT(
