...
class DownLayer(nn.Module):
    ...
    def forward(self, x, pos, pos_embed, H, W, pos_size, N_grid):
        ...
        conf_extra = conf.new_zeros([B, 1, H, W])
        conf_extra[:, :, 0:H:2, 0:W:2] = 1
        conf_extra = map2token(conf_extra, pos, mode='nearest', align_corners=True)

        # only used in top-k sampling.
        conf_topk = conf + conf_extra
        if vis:
            if H == 56:
                show_conf(conf_topk, pos)
        ...
        index_down = gumble_top_k(conf_topk, self.sample_num, 1, T)
...
@register_model
def mypvt13_small(pretrained=False, **kwargs):
    model = MyPVT(
...
if __name__ == '__main__':
    ...
    model = mypvt13_small(drop_path_rate=0.1).to(device)
