...
class MyMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        hidden_features = hidden_features or in_features
        self.fc1 = nn.Linear(in_features, hidden_features)
        self.dwconv = MyDWConv2(hidden_features)
        self.act = act_layer()
        self.fc2 = nn.Linear(hidden_features, out_features)
        self.drop = nn.Dropout(drop)
        self.relu = nn.ReLU(inplace=True)
...
class MyDWConv2(nn.Module):
    def __init__(self, dim=768):
        super().__init__()
        # self.dwconv = nn.Conv2d(dim, dim, 3, 1, 1, bias=True, groups=dim)
        self.dwconv = nn.Conv1d(dim, dim, 10, 10, 0, bias=True, groups=dim)

    def forward(self, x, loc, H, W, kernel_size, sigma):
        B, N, C = x.shape
        x_map = token2map(x, loc, [H, W], kernel_size=kernel_size, sigma=sigma)
        local = extract_neighbor_feature(x_map, loc)
        x = torch.cat([x[..., None], local], dim=-1)
        x = self.dwconv(x.flatten(0, 1)).squeeze(-1)
        x = x.reshape(B, N, C)
        return x


class MyConv(nn.Module):
    def __init__(self, dim_in, dim_out, kernel_size, inter_kernel, inter_sigma):
        super().__init__()
        k = kernel_size * kernel_size + 1
        self.conv = nn.Conv1d(dim_in, dim_out, k, k, 0, bias=True)
        self.inter_kernel = inter_kernel
        self.inter_sigma = inter_sigma

    def forward(self, x, loc, H, W):
        B, N, C = x.shape
        x_map = token2map(x, loc, [H, W], kernel_size=self.inter_kernel, sigma=self.inter_sigma)
        local = extract_neighbor_feature(x_map, loc)
        x = torch.cat([x[..., None], local], dim=-1)
        x = self.conv(x.flatten(0, 1)).squeeze(-1)
        x = x.reshape(B, N, -1)
        return x
...
class ResampleBlock(nn.Module):
    def __init__(self,
        ...
        if dim_out != embed_dim:
            self.pre_conv = MyConv(embed_dim, dim_out, 3, inter_kernel, inter_sigma)
        else:
            self.pre_conv = None
...
def extract_neighbor_feature(src, loc, kernel_size=(3, 3)):
    ...
    grid = loc[:, :, None, None, :] + grid[None, None, ...]     # (B, N, h, w, 2)
    loc_feature = F.grid_sample(src, grid.flatten(2, 3))        # (B, C, N, h * w)
    loc_feature = loc_feature.permute(0, 2, 1, 3)               # (B, N, C, h * w)
    return loc_feature
...
@register_model
def mypvt25b_small(pretrained=False, **kwargs):
    model = MyPVT25b(
...
if __name__ == '__main__':
    ...
    model = mypvt25b_small(drop_path_rate=0.1).to(device)
