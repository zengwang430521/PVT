...
class GridDown(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        x = self.proj(x)
        x = x[:, :, 0:H:self.patch_size[0], 0:W:self.patch_size[1]]
...
class AttenDown(Attention):

    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, attn_cfg=None):
    ...
    def forward(self, x):
        ...
        loc_y, loc_x = torch.meshgrid(
            torch.linspace(0, 1, H, dtype=x.dtype, device=x.device),
            torch.linspace(0, 1, W, dtype=x.dtype, device=x.device),
        )
        loc_source = torch.stack([loc_x, loc_y], -1)
        loc_source = loc_source[None, ...].repeat([B, 1, 1, 1])
        loc = loc_source[:, 0:H:self.patch_size[0], 0:W:self.patch_size[1], :]
        loc_source = loc_source.reshape(B, H * W, 2)
        loc = loc.reshape(B, -1, 2)
        dist = loc[:, :, None, :] - loc_source[:, None, :, :]
        dist = dist.norm(p=2, dim=-1)
        att_mask = (dist < 1.5 / H).type(x.dtype)
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.patch_embed2 = AttenDown(
            ...
            attn_cfg=dict(
               ...
               attn_drop=attn_drop_rate, sr_ratio=1)
        )
...
@register_model
def pvt4_small(pretrained=False, **kwargs):
...
if __name__ == '__main__':
    ...
    model = pvt4_small(drop_path_rate=0.1).to(device)
