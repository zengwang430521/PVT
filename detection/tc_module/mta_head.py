...
class BiMTA(BaseModule):
    def __init__(self,
        ...
        for i in range(self.start_level, self.end_level):
            ...
            # down layers for feature maps
            # use dwconv for less parameters
    ...
    def forward(self, inputs):
        ...
        # extra down conv for details inside tokens
        for i in range(len(input_dicts)-1):
            outs[i+1] += self.down_convs[i](outs[i])
