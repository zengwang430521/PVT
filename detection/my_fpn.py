...
def test_token2map():
    hl, wl = 4, 4
    f_map = torch.rand([2, 3, hl, wl])
    x = f_map.reshape(2, 3, -1).permute(0, 2, 1)
    loc = torch.arange(hl*wl)[None, :].repeat([2, 1])

    y_map, x_map = torch.meshgrid(torch.arange(hl).float() / (hl-1), torch.arange(wl).float() / (wl-1))
    xy_map = torch.stack((x_map, y_map), dim=-1)
    loc_xy = xy_map.reshape(-1, 2)[loc, :]

    map_size = (4, 4)
    out = token2map(x, loc_xy, map_size, kernel_size=3, sigma=2.5)
    err = out - f_map

    map_size = (2, 2)
    out2 = token2map(x, loc_xy, map_size, kernel_size=3, sigma=2.5)
    err2 = out2 - torch.nn.AvgPool2d(2)(f_map)

    map_size = (8, 8)
    out3 = token2map(x, loc_xy, map_size, kernel_size=5, sigma=2.5)
    err3 = out3 - F.interpolate(f_map, [8, 8], mode='bilinear')
    t = 0
...
class MyFPN(BaseModule):
    ...
    def forward(self, inputs):
        ...
        locs = [torch.cat(tmp[1], dim=1) for tmp in inputs]
        feature_sizes = [tmp[2] for tmp in inputs]

        # build laterals in token format
        lateral_tokens = [
            lateral_conv(tokens[i + self.start_level])
        ...
        for i in range(used_backbone_levels - 1, -1, -1):
            ...
            laterals[i] = token2map(lateral_tokens[i], locs[i+self.start_level], map_size, kernel_size, sigma)
            for j in range(i+1, used_backbone_levels):
                laterals[i] += token2map(lateral_tokens[j], locs[j + self.start_level], map_size, kernel_size, sigma)
...
class MyFPN2(BaseModule):
    ...
    def forward(self, inputs):
        ...
        laterals = [
            lateral_conv(
                token2map(tokens[i + self.start_level],
                          locs[i+self.start_level],
                          feature_sizes[i+self.start_level],
                          self.kernel_size[i+self.start_level],
                          self.sigma[i+self.start_level])
            )
            for i, lateral_conv in enumerate(self.lateral_convs)
        ]
...
'''token fpn with inter points'''
@NECKS.register_module()
class TokenFPN(BaseModule):
    r"""Feature Pyramid Network.
