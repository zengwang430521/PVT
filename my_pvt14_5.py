...
class DownLayer(nn.Module):
    ...
    def forward(self, x, pos, pos_embed, H, W, pos_size, N_grid):
        ...
        x_down_ada = torch.gather(x_ada, 1, index_down.expand([B, self.sample_num, C]))
        pos_down_ada = torch.gather(pos_ada, 1, index_down.expand([B, self.sample_num, 2]))

        hg, wg = math.ceil(H/2), math.ceil(W/2)
        x_grid = x_grid.reshape(B, hg, wg, C)
        x_down_grid = x_grid[:, 0:hg:2, 0:wg:2, :].reshape(B, -1, C)
        pos_down_grid = pos_grid.reshape(B, hg, wg, 2)[:, 0:hg:2, 0:wg:2, :].reshape(B, -1, 2)
        N_grid = x_down_grid.shape[1]

        x_down = torch.cat([x_down_grid, x_down_ada], 1)
        pos_down = torch.cat([pos_down_grid, pos_down_ada], 1)
        x_down = self.block(x_down, x, pos, H, W, conf)

        pos_feature = get_pos_embed(pos_embed, pos_down, pos_size)
        x_down += pos_feature
        x_down = self.pos_drop(x_down)
        return x_down, pos_down, N_grid
...
class MyPVT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # stage 2
        x, loc, N_grid = self.down_layers1(x, loc, self.pos_embed2, H, W, self.pos_size, N_grid)     # down sample
        H, W = H // 2, W // 2
        ...
        # stage 3
        x, loc, N_grid = self.down_layers2(x, loc, self.pos_embed3, H, W, self.pos_size, N_grid)     # down sample
        H, W = H // 2, W // 2
        ...
        # stage 4
        x, loc, N_grid = self.down_layers3(x, loc, self.pos_embed4, H, W, self.pos_size, N_grid)     # down sample
        H, W = H // 2, W // 2
...
def show_tokens(x, out, N_grid):
    ...
    for i in range(1):
        ...
        for lv in range(len(out)):
            ...
            H, W = out[lv][2]
            N_grid = math.ceil(H/2) * math.ceil(W/2)
...
@register_model
def mypvt14_4_small(pretrained=False, **kwargs):
    model = MyPVT(
...
if __name__ == '__main__':
    ...
    model = mypvt14_4_small(drop_path_rate=0.1).to(device)
